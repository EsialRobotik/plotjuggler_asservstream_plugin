...
bool Cborg::getFloat(float* value) const
{
    CborgHeader head;
    head.decode(cbor);

    if (head.getMajorType() == CborBase::TypeSpecial && head.getMinorType() == CborBase::TypeSingleFloat)
    {
        uint32_t raw  = head.getValue();
        *value = ((float*)&raw)[0]; 
        return true;
    }
    else
                ...
                case CborBase::TypeSpecial:
                    {
                        ...
                        {
                            Cborg object(&cbor[progress], maxLength - progress);
                            float singlePrecision = -1;
                            bool result = object.getFloat(&singlePrecision);
                            if( result)
                                printf("%f\r\n",singlePrecision);
                            else
                             printf("NaN\r\n");
